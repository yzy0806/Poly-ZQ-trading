...
export function ExecutionPanel({ batch }: { batch: BatchView }) {
  return <Panel title="Execution & hedge monitor" eyebrow="ONE ACTIVE 10-CONTRACT BATCH">
    ...
    <div className="obligations">{batch.obligations.length ? batch.obligations.map((item) => <div key={item.obligation_id}><span>{item.token_id.slice(0, 12)}… · {item.state}</span><b>{number(item.confirmed_shares)} / {number(item.due_shares)}</b><em>{number(item.deficit_shares)} unconfirmed · {number(item.pending_shares ?? 0)} settling · {number(item.excess_shares ?? 0)} excess · ask limit {number(item.latest_limit_price, 4)} · {item.reprice_count} attempt(s)</em></div>) : <div className="empty compact">No hedge obligations.</div>}</div>
  </Panel>
}
