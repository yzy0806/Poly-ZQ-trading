...
function ScenarioPath({ label, row }: { label: string; row: ScenarioPnl }) {
  return <div className="scenario-path">
    ...
    <div><span>Net P&amp;L</span><code>{signedUsd(row.futures_pnl, 2)} + {signedUsd(row.polymarket_pnl, 2)} − {usd(row.costs, 2)} costs − {usd(row.reserves, 2)} reserves</code><strong className={tone(row.net_pnl)}>{signedUsd(row.net_pnl, 2)}</strong></div>
  </div>
}
...
function ScenarioCalculation({ passive, emergency }: { passive: ScenarioPnl; emergency: ScenarioPnl }) {
  return <article className="scenario-calculation">
    ...
    <div className="scenario-path-grid">
      <ScenarioPath label="PASSIVE-POST HEDGE" row={passive} />
...
function CalculationAudit({ opportunity }: { opportunity: Opportunity }) {
  ...
  return <details className="calculation-audit" open>
    ...
    <div className="calculation-foundation">
      ...
      <div><span>Conservative minimum</span><code>min({signedUsd(opportunity.passive_minimum_net_profit, 2)} passive, {signedUsd(opportunity.emergency_minimum_net_profit, 2)} emergency)</code><strong className={tone(opportunity.minimum_net_profit)}>{signedUsd(opportunity.minimum_net_profit, 2)}</strong></div>
    ...
    <div className="cost-formulas">
      <div><span>Explicit costs</span><code>IBKR {usd(costs.ibkr_commission, 2)} + Poly fees {usd(costs.polymarket_fees, 2)} + ZQ slippage {usd(costs.zq_slippage_reserve, 2)} + Poly slippage {usd(costs.polymarket_slippage_reserve, 2)} + rounding {usd(costs.rounding_reserve, 2)}</code><strong>{usd(costs.explicit_costs, 2)}</strong></div>
      <div><span>Risk reserves</span><code>model {usd(costs.model_reserve, 2)} + operational {usd(costs.operational_reserve, 2)} + EFFR basis {usd(costs.effr_basis_reserve, 2)}</code><strong>{usd(costs.reserves, 2)}</strong></div>
    </div>
...
function OpportunityCard({ opportunity }: { opportunity: Opportunity }) {
  ...
  return <article className="opportunity long-only-opportunity">
    ...
    <div className="hedge-legs">{Object.entries(opportunity.token_requirements).map(([token, shares]) => <div key={token}><span>{token} YES</span><b>{number(shares, 2)} shares</b><small>post {number(opportunity.token_prices[token], 4)} · emergency VWAP {number(opportunity.emergency_token_prices[token], 4)}</small></div>)}</div>
    {!!opportunity.hedge_depth.length && <div className="hedge-depth-grid">{opportunity.hedge_depth.map((depth) => <div key={depth.leg_code} className={depth.sufficient && depth.maker_price !== null ? 'depth-pass' : 'depth-fail'}><b>{depth.leg_code}</b><span>Required <strong>{number(depth.required_shares, 2)}</strong></span><span>Available inside cap <strong>{number(depth.available_shares, 2)}</strong></span><span>Shortfall <strong>{number(depth.shortfall_shares, 2)}</strong></span><span>Emergency cap <strong>{number(depth.price_cap, 4)}</strong></span></div>)}</div>}
